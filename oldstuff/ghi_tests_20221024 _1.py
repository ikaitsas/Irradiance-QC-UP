...
# mhpws kalytera na ta kanw me values (0 or 1, -1 or 1)??
# an kai ta boolean den diatypwnontai ws 0 kai 1?? tha dw..
...
print('PPL fail percentage:')
print(ppl_fail_p)
print('ERL flag percentage:')
...
thelw na vrw posa datapoints ksepernoun ta panw oria mono (h ta katw)
'''
...
df2['closr'] = df2['closr'].mask(df2['sumw'] < 50)
# na kanw mask() kai gia Z>93 kalytera?? mhn mplekw parakatw..

# sto ena paper exei ghi>50 kai sto allo sumw>50, ti na valw? tha rwthsw..
plt.scatter( df2['Z'][df2['Z']<93], df2['closr'][df2['Z']<93], s=0.001, c='k')
'''
...
df2['dif_r'] = df2['dif_r'].mask(df2['GH'] < 50)

plt.scatter( df2['Z'][df2['Z']<93], df2['dif_r'][df2['Z']<93], s=0.001, c='k') 
# ratio limits visualization
...
df2['closr_fail'] = ((df2['closr'][df2['Z']<75]>1.08) | \
                     (df2['closr'][df2['Z']<75]<0.92) | \
                     (df2['closr'][(df2['Z']<93) & (df2['Z']>75)]>1.12) | \
                     (df2['closr'][(df2['Z']<93) & (df2['Z']>75)]<0.85) )             
df2['closr_fail'] = df2['closr_fail'].mask(df2['closr'] == np.nan, other=np.nan)
print(df2['closr_fail'].value_counts())
# gia Z<93 alla me closr=nan exw kapoia false, auta prepei na afairethoun
# na to diorthwsw auto to probhma, h na kanw assign values, oxi booleans..
# an kanw mask() kai gia Z<93 isws na lythei?? tha to dw..

df2['dif_fail'] = ((df2['dif_r'][df2['Z']<75]>1.05) | \
                   (df2['dif_r'][(df2['Z']<93) & (df2['Z']>75)]>1.15))
# den vriskw lysh sto problhma, mallon tha kanw assign values (1, 0, nan)..
